...
verified CSV and probes read-only Kaggle history/API metadata for ref 56278642.
"""
...
def audit_submission(path,expected_ids=None):
    ...
    seen=set();result['duplicate_ids']=sorted({v for v in ids if v in seen or seen.add(v)})
...
def main():
    ...
    # Compare IDs to current visible template and current visible test groups.
    sample_ids=[]
    ...
    test_unique=list(dict.fromkeys(test_ids))
    audit=audit_submission(verified,expected_ids=sample_ids or test_unique)
    audit['test_id_set_matches']=set(r for r in test_unique)==set(r for r in sample_ids) if sample_ids else None
    audit['test_unique_ids']=len(test_unique);audit['sample_ids']=len(sample_ids)
    # If example test carries labels, score it locally purely as a diagnostic.
    schema=pq.ParquetFile(data/'test.parquet').schema_arrow
    local_metric=None
    if 'normalized_smiles' in schema.names:
        table=pq.read_table(data/'test.parquet',columns=['molecule_id','normalized_smiles'])
        targets={}
    ...
    match=[r for r in rows if int(r.get('ref',-1))==REF]
    row=match[0] if len(match)==1 else None
    verbose=cli('history_verbose',['competitions','submissions',SLUG,'-v'])
    help_text=cli('competitions_help',['competitions','--help'])
    # Inspect installed API method names and, when possible, raw submission objects.
    api_info={'methods':[],'raw_submission':None,'error':None}
    try:
        ...
        if hasattr(api,'competition_submissions'):
            objects=api.competition_submissions(SLUG)
            for obj in objects:
                values={}
                for name in dir(obj):
                ...
                if str(values.get('ref',values.get('id','')))==str(REF):api_info['raw_submission']=values;break
    except Exception as exc:api_info['error']=type(exc).__name__+': '+str(exc)[:1000]
    result={'checked_utc':dt.datetime.now(dt.timezone.utc).isoformat(),'submission_ref':REF,'description':DESCRIPTION,
            ...
            'csv_audit':audit,'local_example_metric':local_metric,'verbose_history':verbose,'competitions_help':help_text,
            'api_info':api_info,
            'kaggle_scoring_error_interpretation':'Kaggle UI screenshot reports Submission Scoring Error; CLI COMPLETE without a score is not treated as successful scoring.'}
